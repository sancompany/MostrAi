...
| GET | `/planos-ponto` | **Aposentada em 24/09/2026 (ADR-016)**: devolve `[]` — ser ponto não tem modalidade; gera +1 crédito por mês. Mantida só pra tela antiga aberta não quebrar. |
...
| GET | `/admin/dispositivos/:id` | ficha completa de uma tela |
...
| POST | `/admin/dispositivos/:id/provisionamento` | gera o `mostrai-config.json` `{baseUrl, tokenProvisionamento, rotacaoTela}` — token só nesta resposta; cancela o anterior não usado; não derruba o Player atual |
...
| POST | `/admin/dispositivos/:id/chave-legada` | compat-v1: chave do player web, devolvida uma vez dentro do link `player.html?tela=&chave=` |
