...
const CAMPOS_ATUALIZAVEIS = ['nome', 'chamada', 'ajuda_custo_mensal', 'cota_slots_hora', 'beneficios', 'ordem', 'ativo',
  'plano_bonus_id', 'plano_bonus_apos_meses', 'plano_bonus_meses'];
...
async function atualizar(id, dados) {
  ...
  const { rows } = await pool.query(
    `UPDATE planos_ponto SET ${sets} WHERE id = $1 RETURNING *`,
...
async function criar(dados) {
  const { rows } = await pool.query(
    ...
    [dados.id, dados.nome, dados.chamada, dados.ajuda_custo_mensal || 0,
      dados.cota_slots_hora || 0, dados.beneficios || [], dados.ordem || 0]
  );
  return rows[0];
}
