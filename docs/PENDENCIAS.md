...
**Segunda colisão de número, no merge seguinte (mesmo dia, PR #20 com
`main`):** a rodada de separação comodato × plano comercial (seção acima
deste documento, ".ia/HANDOFF.md") tinha migrations próprias `076`/`077`
(`separa_comodato_de_plano_comercial`/`suspensao_so_manual...`), criadas em
paralelo a esta rodada — que já tinha renumerado sua própria `074` pra
`076_horario_operacional_da_tela.sql`, como a nota logo acima registra. As
duas nunca tinham colidido de fato (nomes de arquivo diferentes, o runner de
migração — `src/db/migrate.js` — trava por nome completo, não pelo prefixo
numérico), mas duas migrations `076_*` no mesmo diretório quebra a
convenção de sequência única que todo o resto do projeto segue. Renumeradas
pra `077_separa_comodato_de_plano_comercial.sql` e
`078_suspensao_so_manual_cobertura_vence_cancela.sql` antes do merge —
nenhuma das duas tinha sido aplicada em produção ainda, então é rename
seguro (`CONSTRAINTS.md`: a regra é não editar migration JÁ aplicada).
Comentários que citavam "migration 076"/"migration 077" nesse contexto
...
são segredo, mas CONSTRAINTS.md pede fora do repositório).
