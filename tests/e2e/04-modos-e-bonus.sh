...
$PG -c "UPDATE anunciantes SET plano_id='essencial-3m', status='ativo', data_inicio_cobertura = now() - interval '4 months', data_expiracao = now() + interval '2 months' WHERE id=$BETO" >/dev/null
...
$PG -c "UPDATE anunciantes SET plano_id=NULL, status='aprovado', data_expiracao=NULL WHERE id=$LIA" >/dev/null
