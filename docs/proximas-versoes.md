...
## Conta própria vira preenchimento elástico do vazio, configurada por criativo (não por conta)
- **O que:** duas mudanças ligadas, adiadas para a rodada de revisão do admin. (1) A conta própria deixa de competir por uma frequência fixa (`frequencia_hora_propria`) e passa a **absorver o que sobrar** depois dos anunciantes pagos — hoje um ponto com 3% de ocupação paga tem 97% da hora virando cartão institucional estático; o dono quer que esse excedente seja, antes de mais nada, espaço da própria rede. (2) Configuração deixa de ser por conta (uma frequência única valendo pra toda a conta própria) e passa a ser **por criativo**: cada peça da conta própria ganha seu próprio alcance (todos os pontos ou um subconjunto) e sua própria frequência/"plano" — nas palavras do dono, "não tem como eu falar uma frequência, porque isso vai bater junto com os anunciantes"; ele quer o equivalente de um plano por anúncio, não por negócio.
- **Por que:** o desenho atual (RN-40, `docs/funcional.md`) trata a conta própria como "mais um anunciante" de frequência fixa, e frequência fixa só funciona quando o vazio da hora é prevísivel — numa rede nova, com ocupação paga baixa e variável por ponto, uma frequência fixa não absorve o vazio real (sobra demais) nem se ajusta ponto a ponto.
- **De onde veio:** o dono, 21/09/2026, depois de uma investigação desta sessão que confirmou como o sistema funciona hoje (conta própria com frequência fixa competindo com pagantes; cartão institucional preenchendo todo o resto) — ele confirmou que quer mudar o desenho, não que havia um bug no que já foi construído.
- **O que toca:** motor de orçamento da hora (`montarHoraDeTv`/`espalhar`, `src/lib/pacing.js`) e a consulta de elegibilidade (`anunciantesElegiveis`, `src/playlist/gerador.js:82-123`, hoje lê `frequencia_hora_propria` da conta) — passam a decidir o excedente depois do corte dos pagantes, não numa fatia fixa; tabela `criativos` ganha campos de alcance/frequência próprios para os da conta própria (hoje esses campos não existem — frequência é da conta, não do criativo); admin (`public/admin/index.page.js`) ganha a UI pra configurar isso por criativo. **Correção de 21/09/2026:** uma versão anterior desta entrada dizia que a aba "Meus anúncios" do admin estava quebrada (listando só `status=pendente` enquanto os criativos da conta própria nascem `aprovado`). **Está errado** — só o *default* da rota é `pendente` (`src/admin/routes.js:16`); o call-site da aba pede `?status=todos` explicitamente (`public/admin/index.page.js:718`) e `listarPorStatus` trata esse caso (`src/anunciantes/criativos-repository.js:52-56`). O bug existiu e foi corrigido no commit `7b4d173`, de 15/09/2026. Se a aba aparecer vazia, a causa é outra: conta própria ainda não criada, ou nenhum criativo enviado pra ela.
- **Quando vale a pena:** quando a sessão entrar na revisão do painel admin (já listada em `.ia/HANDOFF.md`, "Revisão visual/funcional das telas") — o dono pediu explicitamente pra discutir isso "um pouquinho mais pra frente", não construir agora.
