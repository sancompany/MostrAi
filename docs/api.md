...
| GET | `/anunciantes/:id/exibicoes` | O que rodou pro anunciante, por ponto e por dia. Com plano ativo, também devolve `horasContratadasMes`/`horasEntreguesMes` (horas contratadas no mês vs. já entregues, ilustrativo — mesma aproximação do banco de horas); `null` sem plano. |
