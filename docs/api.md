...
| GET | `/anunciantes/me/creditos` | Ledger de créditos e benefícios (migration 079, 23/09/2026): `{saldo, opcoes, movimentacoes, beneficioAtivo, beneficioAgendado}`. `opcoes` são as 12 combinações tier×período (`src/creditos/regras.js`) com `disponivel`/`faltam`. Alimenta o card "Créditos e benefícios" do painel numa chamada só. |
| POST | `/anunciantes/me/creditos/resgatar` | `{tier, meses}` → `{conta, status:'ativo'\|'agendado'}`. Débito do ledger e criação/agendamento do benefício numa transação — nunca existe débito sem benefício nem o contrário. Um ciclo pago em curso NUNCA é interrompido: se a conta ainda está pagando em dia, o resgate fica `agendado` e só entra em vigor quando o ciclo atual vencer (reavaliação diária). 409 saldo insuficiente, conta suspensa, ou comodato Inicial (mesma trava do plano comercial direto). |
...
| GET | `/conta/eventos` | Server-Sent Events (`text/event-stream`) — eventos em tempo real da conta logada (`payment.updated`, `credits.updated`, `notification.created`, mais os que forem emitidos conforme o resto do site adota `src/lib/sse.js`). Heartbeat a cada 25s. Isolado por `anuncianteId` da sessão — nunca vaza evento de outra conta. |
