...
| GET | `/anunciantes/me/financeiro` | **Conta logada** (Fatia 4 do painel único, 23/09/2026): os dois sentidos do dinheiro. `pagamentos` — `mostrar` (tem plano ou já pagou algo), `plano` `{nome, situacao: ativa/cortesia/vencida/suspensa/sem_plano, validoAte}`, `cobrancas[]` `{id, data, valor}` (últimas 36). `recebimentos` — `mostrar` (algum ponto recebe ajuda de custo ou já tem lançamento), `ajudaMensal`, `resumo` (mesmo `resumir` do extrato), `linhas[]` `{id, competencia, ponto, valor, pagoEm, forma}` — sem a `observacao` interna do lançamento —, e `troca` `{totalMensal, pontos}` enquanto algum ponto ainda recebe em dinheiro (a troca é `POST /anunciantes/me/comodato/trocar-por-tela`, que passa a emitir `point.updated`, `finance.updated` e `plan.updated`). |
...
| POST | `/admin/anunciantes/:id/plano-administrativo/encerrar` | encerra só o plano COMERCIAL concedido agora — o comodato (`comodato_plano_id`) nunca é tocado, nem "volta": ele nunca deveria ter desaparecido (23/09/2026, correção do modelo de domínio). 400 se o plano vigente não for cortesia (assinatura paga usa `cancelar-assinatura`). 409 se o benefício em vigor foi pago com créditos. Sem tela desde a revisão da ficha de Conta (23/09/2026) |
