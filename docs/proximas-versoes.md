...
- **O que toca:** motor de orçamento da hora (`montarHoraDeTv`/`espalhar`, `src/lib/pacing.js`) e a consulta de elegibilidade (`anunciantesElegiveis`, `src/playlist/gerador.js:82-123`, hoje lê `frequencia_hora_propria` da conta) — passam a decidir o excedente depois do corte dos pagantes, não numa fatia fixa; tabela `criativos` ganha campos de alcance/frequência próprios para os da conta própria (hoje esses campos não existem — frequência é da conta, não do criativo); admin (`public/admin/index.page.js`) ganha a UI pra configurar isso por criativo. **Achado à parte, no mesmo caminho:** a aba "Meus anúncios" do admin só lista criativos com `status=pendente` (`src/admin/routes.js:16`), mas os da conta própria já nascem `aprovado` — hoje eles nunca aparecem lá, o que pode ser por que a conta própria parece "vazia" mesmo se tiver algo cadastrado. Vale corrigir esse filtro junto, é baixo risco e independente do resto.
