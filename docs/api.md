...
| Método | Rota | O que faz |
|---|---|---|
| GET | `/admin/pontos` | lista (inclui `telas_instaladas` — dispositivos com `instalado_em` preenchido, base do status visual "TV instalada" no admin) |
| PATCH | `/admin/pontos/:id` | status, endereço, ajuda de custo, cota, `horario_semanal`, `observacoes` (migration 067). Admin edita só o painel de Instalação (status/`acabamento_completo`) desde o redesenho de 22/09/2026 — o resto da ficha é somente-leitura na tela, mas o PATCH continua aceitando qualquer campo da whitelist se chamado direto. |
