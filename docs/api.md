...
| GET | `/anunciantes/:id/exibicoes` | O que rodou pro anunciante, por ponto e por dia. Com plano ativo, também devolve `horasContratadasMes`/`horasEntreguesMes` (horas contratadas no mês vs. já entregues, ilustrativo — mesma aproximação do banco de horas) e `custoPorHora` (valor mensal pago ÷ horas contratadas no mês — `null` em cortesia ou sem plano); os três `null` sem plano. |
