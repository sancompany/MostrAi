...
| POST | `/admin/dispositivos/:id/credencial/rotacionar` | cria credencial candidata (vai no heartbeat até o Player usar; a atual vale até lá e a anterior por 24 h) |
...
| POST | `/admin/dispositivos/:id/pin` | PIN de manutenção do Player (exatamente 4 dígitos; cifrado para a config do V2 e com hash para o player web). `null` remove |
