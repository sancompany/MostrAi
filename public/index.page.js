...
// fora o fundador, que so aparece enquanto o programa esta aberto. E o
// primeiro degrau da grade, que e o que "a partir de" quer dizer.
fetch(`${API_BASE_URL}/planos`).then((r) => r.json()).then((planos) => {
  const mensais = (planos || []).filter((p) => p.ativo && !p.fundador && Number(p.compromisso_meses) === 1);
