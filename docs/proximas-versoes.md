...
## Trocar a unidade de frequência de "por dia" para "por hora", com folga de 15 min
- **O que:** `planos.frequencia_dia` **já é** um campo numérico livre, editável por plano no admin, e já funciona de ponta a ponta (isso não muda). **Mas o dono pediu, 15/09/2026, pra trocar a unidade**: em vez de configurar "quantas vezes por dia", configurar "quantas vezes por hora" diretamente — com liberdade de a exibição acontecer até 15 minutos antes ou depois da hora cheia (dentro dessa folga de ±15 min ao redor do período). Ele não detalhou se isso substitui `frequencia_dia` ou vira um campo novo ao lado — checar com ele na hora de desenhar.
- **Por que:** pedido do dono; parece a mesma folga de 15 minutos já registrada na entrada abaixo ("Folga de 15 minutos pro déficit da hora anterior") — dá pra ler como a MESMA ideia dita de dois jeitos, ou como duas coisas relacionadas (uma é a unidade de configuração do plano, a outra é o motor de recuperação de déficit). Não juntar as duas sem confirmar com o dono qual é a intenção real.
- **De onde veio:** pedido do dono, 15/09/2026, na rodada de depuração da página de Planos.
- **O que toca:** `planos.frequencia_dia` e o cálculo de `frequenciaBase` (hoje `Math.ceil(frequencia_dia / horasAberto)`) em `gerarPlaylistDaHora` (`src/playlist/gerador.js`); o formulário do admin (`renderPlanos`, campo "Frequência/dia"); possivelmente a mesma folga de 15 min de `calcularPlaylist` (`src/lib/pacing.js`) da entrada abaixo.
- **Quando vale a pena:** perguntar ao dono antes de construir — junto com a entrada de baixo, pra não implementar duas vezes a mesma folga de 15 minutos.
