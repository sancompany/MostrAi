...
function gerarCupom(nome) {
  const slug = String(nome || 'vend').normalize('NFD').replace(/[̀-ͯ]/g, '')
    .replace(/[^a-zA-Z]/g, '').toUpperCase().slice(0, 6) || 'VEND';
...
async function atualizar(contaId, dados) {
  ...
  const { rows } = await pool.query(
    `UPDATE vendedores SET ${sets} WHERE conta_id = $1 RETURNING *`, [contaId, ...campos.map((c) => dados[c])]
  );
  return rows[0] || null;
}
