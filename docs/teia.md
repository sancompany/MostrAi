...
**planos.page.js — render(meses) da grade normal** — Filtra `compromisso_meses === meses && !fundador` e monta um .plan-card por plano com: badge "Mais escolhido" (destaque_no_site), rótulo, nome do tier, "Nx por dia em cada ponto (≈Mx por hora)", preço cheio riscado (referência × meses), total do ciclo em…
  ...
  · **cliente sabe:** Bem explicado no preço: o riscado, o total do ciclo e a economia por mês estão todos na tela. NÃO explicado: (a) o "≈Mx por hora" é `frequencia_dia / 12` — a jornada de 12 h é presumida em código e…
...
- "≈Nx por hora" em planos.page.js é `Math.round(p.frequencia_dia / 12)`. A jornada de 12 horas está codificada e nunca é dita ao cliente — em ponto com horário diferente (`horario_abertura`/`horario_fechamento` existem na tabela `pontos`) o número mostrado na vitrine não corresponde.
...
**Select de ramo de atividade (categorias)** — ligarCategorias(escopo) em formulario.js busca GET /categorias (src/categorias/routes.js: `SELECT id, nome FROM categorias WHERE ativo ORDER BY nome`, rota pública sem sessão) e popula todo <select data-categorias>. Se a categoria escolhida tiver data-nome…
...
**Nascimento do ponto e da Tela 1 (transação do cadastro)** — Dentro de uma transação única em POST /anunciantes/cadastro: consome o convite ANTES de criar a conta, cria a conta com papeis do convite e status 'aprovado', e — se papeis inclui 'ponto' e convite.candidatura_id existe — chama criarPontoDaCandidatura(). Essa…
...
**Convite aceito por quem já tem conta** — Se carregarConta() devolve sessão, convite.page.js troca o formulário pelo card #jaLogado e chama POST /convites/:token/aceitar. O handler em modos.js roda emTransacao(): consome o token, busca a candidatura ligada e chama liberarPapelNaConta() para cada…
...
**Pedido do modo 'Meu ponto' de dentro do painel** — Conta que já existe pede uma tela sem convite. montarModo('ponto', ...) em modos.js desenha o card CARDS.ponto com nome_comercio, endereço completo, segmento, fluxo estimado, os radios de comodato (#modoEscolhaPlano, carregados por carregarOpcoesComodato) e…
...
**Confirmação de plano e ida ao checkout** — Com ?plano=X na URL e a conta sem plano_id, confirmarPlano(planoId) busca GET /planos, acha o plano e mostra um bloco 'Confirmar assinatura' com nome, frequencia_dia, total (valor_mensal × compromisso_meses) e 'Preço travado por N meses' quando preco_travado.…
...
- Custo por exibição ignora o preço travado e a cortesia: em GET /anunciantes/:id/exibicoes o cálculo é `Number(plano.valor_mensal) * plano.compromisso_meses / confirmadas`, enquanto a cobrança real usa `valorMensalDaConta(anunciante, plano)` (src/financeiro/san-checkout.js:135), que prefere…
...
**Cobranças** — `renderCobrancas(el)` (linha 1523). Pagamentos confirmados e emissão de nota fiscal. Dois KPIs (Total confirmado / Notas por emitir). Tabela: ID, Anunciante, Valor, Data e Nota fiscal — quando `nota_fiscal_status='emitida'`, selo verde + link 'ver PDF';…
...
**Comissões** — `renderComissoes(el)` (linha 1559). KPI 'Total a pagar' mais até três KPIs, um por vendedor (agrupamento client-side em `porVendedor`, mostrando total, quantidade e a chave Pix — ou 'sem chave Pix'). Tabela: Vendedor, Anunciante, Venda (`valor_confirmado`),…
...
**Devoluções (arrependimentos)** — `renderArrependimentos(el)` (linha 1601). Quem desistiu da contratação dentro dos 7 dias da lei. KPI 'A devolver' com o total em aberto. Tabela: Protocolo, Anunciante (+ e-mail), CPF/CNPJ, Valor a estornar, Pedido em, Situação (selo 'a devolver' ou 'devolvido…
...
**Custos fixos** — `renderCustos(el)` (linha 1039). Três KPIs: 'Custos fixos ativos' (somado no cliente, só os `ativo`), 'Amortização das telas' (lido de `RESUMO.financeiro.amortizacaoMensal`, com a legenda 'vem do custo de cada tela (aba Telas)') e 'Ajuda de custo aos pontos'…
...
**Pacing — frequência por hora ajustada ao horário do ponto** — O plano vende 'vezes por dia'; `horasAbertoPorDia(ponto)` em `src/playlist/gerador.js` converte `horario_abertura`/`horario_fechamento` em horas (`(hF*60+mF)-(hA*60+mA))/60`), caindo em `HORAS_ABERTO_PADRAO = 12` se faltar horário ou se der <= 0. A taxa por…
  ...
  · **cliente sabe:** Parcialmente: /planos.html vende 'X vezes por dia'. Não há nenhuma tela que explique que a taxa é recalculada por hora de funcionamento do ponto — está só no comentário do gerador e em…
...
- O player não usa `duracaoSegundos`. O campo é gerado por `src/lib/ffmpeg.js` (com o caso especial de imagem: `DURACAO_PADRAO_IMAGEM = 10`), devolvido pela playlist e documentado em docs/api.md — mas a troca de vídeo é 100% por evento `ended`. O contrato promete um controle que o player ignora.
...
**Versionamento de plano (item 9 / RN-27)** — PATCH só aceita CAMPOS_VITRINE (ativo, destaque_no_site, rotulo, vagas). Campo de CAMPOS_CONTRATO (tier, nome, valor_mensal, valor_mensal_cheio, compromisso_meses, frequencia_dia, cobertura, limite_criativos, preco_travado, fundador, ponto_apos_meses,…
...
**pontos** — O comércio parceiro: endereço, responsável, horário de funcionamento e o contrato de comodato. Colunas que importam: nome, endereco, cidade, uf, cep, segmento (texto livre, legado), categoria_id (FK categorias, é o que bloqueia concorrente na tela),…
  · `src/db/migrations/001_pontos.sql`, `src/db/migrations/012_pontos_status_reparo.sql`, `src/db/migrations/015_beneficios_categorias_planos_ponto.sql` · rotas: `GET /pontos`, `GET /pontos/fluxo`, `POST /seja-um-ponto (410)`, `GET /admin/pontos` · papéis: admin, ponto
...
**candidaturas** — O formulário de quem quer entrar como ponto ou vendedor. tipo CHECK('ponto','vendedor'), nome, nome_comercio, contato_telefone NOT NULL, contato_email, endereco/cidade/uf/cep, segmento, fluxo_estimado_mensal, mensagem, status…
  · `src/db/migrations/019_v2_contas_dispositivos_convites_planos.sql`, `src/db/migrations/020_modos_da_conta_e_modulos_de_plano.sql`, `src/candidaturas/repository.js` · rotas: `POST /candidaturas`, `GET /admin/candidaturas`, `PATCH /admin/candidaturas/:id`, `POST /admin/candidaturas/:id/liberar` · papéis: visitante, anunciante, ponto, vendedor
...
**PATCH /admin/anunciantes/:id** — Status, papéis, dados, conta_propria, frequencia_dia_propria; também é o caminho de restaurar conta excluída (excluido_em: null).
...
**GET /categorias** — Segmentos ativos para os selects de cadastro.
...
**enviarArrependimentoRecebido(anunciante, pedido)** — Assunto "Desistência registrada — Mostraí". Único e-mail com CC para a caixa da Mostraí (MOSTRAI_EMAIL_CONTATO) — porque o estorno é executado à MÃO por uma pessoa no painel do San Checkout/Asaas e sem esse aviso ninguém fica sabendo que há dinheiro a…
...
**planos.html — vitrine de preços** — RESPONDE: quanto custa por ciclo e por mês, quantas vezes por dia o anúncio roda em cada tela (`frequencia_dia`, com a conversão '≈Nx por hora' feita em `planos.page.js`), quanto se economiza escolhendo ciclo maior, quais benefícios cada plano dá (array…
...
**BURACO — plano assinado é imutável para quem assinou (RN-27)** — Campos de contrato (`nome`, `valor_mensal`, `frequencia_dia`, `cobertura`, `limite_criativos`, benefícios) não se editam: publica-se versão nova com id novo e a antiga é aposentada; quem já assinou fica na versão antiga. É uma garantia forte de estabilidade…
