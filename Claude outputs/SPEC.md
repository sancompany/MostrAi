...
| cpf_cnpj | text | pessoa física ou jurídica — vira o `pagador.cpf` do San Checkout e o tomador da nota fiscal |
...
cada combinação tier × ciclo é uma linha própria, ex.: `essencial-mensal`,
`essencial-trimestral`, `destaque-anual`)

| Campo | Tipo | Obs |
|---|---|---|
| id | text PK | ex.: `essencial-trimestral` |
| tier | enum: essencial, destaque, maximo | pra agrupar as 4 linhas de ciclo de um mesmo tier na tela de planos |
| nome | text | |
| valor | numeric | o que é cobrado de fato |
| valor_riscado | numeric nullable | preço "cheio" só pra exibir riscado no site; não afeta cobrança |
| ciclo | enum: mensal, trimestral, semestral, anual | período de cobrança recorrente no San Checkout |
| meses_bonus | int default 0 | meses de cobertura extra além do que foi pago (ex.: trimestral com 1 mês grátis → `meses_bonus=1`) |
| frequencia_hora | int | |
...
- `GET /plano/:planoId` — autenticado por `X-Checkout-Key`, devolve os campos
  do contrato (`planoId, nome, valor, ciclo: MONTHLY, pagador`)
