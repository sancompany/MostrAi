...
- **Uma conta, três modos, um painel.** A tabela `anunciantes` é a tabela de contas (nome histórico). `papeis` ∈ {anunciante, ponto, vendedor}. O painel (`/anunciante/painel.html`, "Painel" no menu — até 19/09/2026 se chamava "Anúncios") é único; só as abas "Anúncios" mostra um card de ativação quando falta o papel (`public/modos.js`). Desde 19/09/2026 "Meu ponto" e "Vendas" não têm mais aba própria no topo — quem ainda não é ponto se candidata por um card simplificado no fim do próprio Painel, e quem já tem qualquer um dos dois papéis acha o painel próprio (`ponto.html`/`vendedor.html`, que continuam existindo) por um link dentro do Painel. Anunciante se cadastra sozinho ou ativa o modo pelo card (só falta o endereço); **dono de ponto e vendedor só entram com liberação do dono**: convite (link de cadastro, ou aceito por conta logada) ou "Liberar na conta" no admin a partir de um pedido feito de dentro do painel.
